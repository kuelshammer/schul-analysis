...
class StatistischeVerteilung:
    ...
    def pdf(self, x: Union[sp.Expr, float]) -> sp.Expr:
    ...
    def cdf(self, x: Union[sp.Expr, float]) -> sp.Expr:
...
class Binomialverteilung(StatistischeVerteilung):
    ...
    def __init__(self, name: str, n: Union[int, sp.Expr], p: Union[float, sp.Expr]):
        super().__init__(name, "Binomial", {"n": n, "p": p})
        self.n = n
        self.p = p
        self.X = stats.Binomial(name, n, p)

    def pdf(self, k: Union[sp.Expr, int]) -> sp.Expr:
        """P(X=k) - Wahrscheinlichkeitsfunktion"""
        return stats.density(self.X)(k)

    def cdf(self, k: Union[sp.Expr, int]) -> sp.Expr:
...
class Normalverteilung(StatistischeVerteilung):
    ...
    def __init__(
        self, name: str, mu: Union[float, sp.Expr], sigma: Union[float, sp.Expr]
    ):
    ...
    def wahrscheinlichkeit_intervall(
        self, a: Union[float, sp.Expr], b: Union[float, sp.Expr]
    ) -> sp.Expr:
        """P(a ≤ X ≤ b)"""
